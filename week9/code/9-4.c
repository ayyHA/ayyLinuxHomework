...
int main(){
	...
	char* string= "test stirng!";
	*pt = 9999;
	printf("%d:----Before vfork-----",getpid());

	pid = vfork();
	if(pid<0){
		fprintf(stderr,"failed to vfork.Reason(%s)",strerror(errno));
		exit(-1);
	}
	else if(pid==0) {
		printf("child : pid =%d, ppid = %d return from vfork()=%d\n",getpid(),getppid(),pid);
	...
	//	exit(0);
		...
		sleep(2);
		printf("parent : pid =%d,ppid =%d return from vfork()=%d\n",getpid(),getppid(),pid);
